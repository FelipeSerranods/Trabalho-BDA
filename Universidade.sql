...
CREATE TABLE ALUNOS ( 
    MATRICULA INT NOT NULL IDENTITY CONSTRAINT PK_ALUNO PRIMARY KEY, 
    NOME VARCHAR(50) NOT NULL 
); 
GO 

CREATE TABLE CURSOS ( 
    CURSO CHAR(3) NOT NULL CONSTRAINT PK_CURSO PRIMARY KEY, 
    NOME VARCHAR(50) NOT NULL 
); 
GO 

CREATE TABLE PROFESSOR ( 
    PROFESSOR INT IDENTITY NOT NULL CONSTRAINT PK_PROFESSOR PRIMARY KEY, 
    NOME VARCHAR(50) NOT NULL 
); 
GO 
...
CREATE TABLE MATERIAS ( 
    ...
    PROFESSOR INT CONSTRAINT PK_MATERIA PRIMARY KEY (SIGLA, CURSO, PROFESSOR) 
...
CREATE TABLE MATRICULA ( 
    ...
    CONSTRAINT FK_ALUNOS_MATRICULA FOREIGN KEY (MATRICULA) 
    REFERENCES ALUNOS (MATRICULA), 
    CONSTRAINT FK_CURSOS_MATRICULA FOREIGN KEY (CURSO) 
    REFERENCES CURSOS (CURSO),   
    CONSTRAINT FK_PROFESSOR_MATRICULA FOREIGN KEY (PROFESSOR) 
    REFERENCES PROFESSOR (PROFESSOR) 
); 
...
SELECT @matriAlunos AS MATRICULA, CURSO, SIGLA, PROFESSOR, YEAR(GETDATE()) AS PERLETIVO FROM MATERIAS WHERE CURSO = @SIGLA 
END  
GO 
...
AS  
BEGIN   
INSERT CURSOS(CURSO, NOME) VALUES(@SIGLA, @CURSO)  
END  
GO   
...
AS  
BEGIN  
INSERT PROFESSOR(NOME) VALUES(@NOMEPROF)  
END  
GO  
...
AS  
BEGIN   
INSERT MATERIAS(SIGLA, NOME, CARGAHORARIA, CURSO, PROFESSOR) VALUES(@SIGLAMAT, @NOMEMAT, @CARGAHORARIA, @SIGLA, @ID_PROF)
END 
GO   

CREATE PROCEDURE procCadastroNotas( 
    @MATRICULA INT, 
    @CURSO CHAR(3), 
    @MATERIA CHAR(3), 
    @PERLETIVO CHAR(4), 
...
ELSE IF @PARAMETRO = 4 
    ...
    DECLARE @RESULTADO VARCHAR(50), 
            ...
            SET N4 = @NOTA, 
                ...
                PERCFREQ = 100 - ((@FALTA + F1 + F2 + F3) * 100) / ((@CARGAHORA * 60) / 45), 
                @MEDIAFINAL = (@NOTA + N1 + N2 + N3) / 4,
...
EXEC procCadastroNotas 1, 'ENG', 'DEW', 2023, 10, 0, 5
...
EXEC procCadastroNotas 1, 'ENG', 'RES', 2023, 10.0, 0, 5
